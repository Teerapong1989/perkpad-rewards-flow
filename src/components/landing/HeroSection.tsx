...
const HeroSection = () => {
  ...
  return (
    <section className="relative overflow-hidden py-8 sm:py-12 lg:py-20 px-4 sm:px-6" role="main">
      {/* Background decoration */}
      <div className="absolute inset-0 bg-gradient-to-r from-teal-50/50 to-orange-50/30 blur-3xl" aria-hidden="true"></div>
      <div className="absolute top-10 sm:top-20 left-4 sm:left-10 w-48 sm:w-72 h-48 sm:h-72 bg-teal-200/20 rounded-full blur-3xl animate-pulse" aria-hidden="true"></div>
      <div className="absolute bottom-10 sm:bottom-20 right-4 sm:right-10 w-64 sm:w-96 h-64 sm:h-96 bg-orange-200/20 rounded-full blur-3xl animate-pulse delay-1000" aria-hidden="true"></div>
      
      <div className="max-w-7xl mx-auto relative">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          <header className="space-y-6 sm:space-y-8 animate-fade-in">
          ...
          <div className="relative animate-fade-in delay-300 mt-8 lg:mt-0">
            ...
            <div className="absolute -top-2 sm:-top-4 -left-2 sm:-left-4 w-12 h-12 sm:w-20 sm:h-20 bg-gradient-to-br from-orange-400 to-orange-500 rounded-2xl shadow-lg animate-bounce delay-500 flex items-center justify-center" aria-hidden="true">
              <Gift className="w-6 h-6 sm:w-10 sm:h-10 text-white" />
            </div>
            <div className="absolute -top-4 sm:-top-8 -right-2 sm:-right-8 w-10 h-10 sm:w-16 sm:h-16 bg-gradient-to-br from-teal-400 to-teal-500 rounded-full shadow-lg animate-bounce delay-700 flex items-center justify-center" aria-hidden="true">
              <Star className="w-5 h-5 sm:w-8 sm:h-8 text-white fill-current" />
            </div>
            <div className="absolute -bottom-3 sm:-bottom-6 -left-3 sm:-left-6 w-8 h-8 sm:w-12 sm:h-12 bg-gradient-to-br from-purple-400 to-purple-500 rounded-xl shadow-lg animate-bounce delay-1000 flex items-center justify-center" aria-hidden="true">
              <Users className="w-4 h-4 sm:w-6 sm:h-6 text-white" />
            </div>
            
            {/* Enhanced phone mockup */}
            <figure className="relative bg-white rounded-3xl shadow-2xl p-4 sm:p-6 lg:p-8 max-w-xs sm:max-w-sm mx-auto transform hover:scale-105 transition-transform duration-300">
                ...
                <div className="grid grid-cols-5 gap-2 sm:gap-3" role="progressbar" aria-label="Loyalty progress: 7 out of 10 stamps collected">
                  {[1,2,3,4,5,6,7].map((i) => (
                    <div key={i} className="w-6 h-6 sm:w-8 sm:h-8 lg:w-10 lg:h-10 bg-gradient-to-br from-teal-100 to-teal-200 rounded-full flex items-center justify-center transform hover:scale-110 transition-transform duration-200">
                      <Check className="w-3 h-3 sm:w-4 sm:h-4 lg:w-5 lg:h-5 text-teal-600" aria-label={`Stamp ${i} collected`} />
                    </div>
                  ))}
                  {[8,9,10].map((i) => (
                    <div key={i} className="w-6 h-6 sm:w-8 sm:h-8 lg:w-10 lg:h-10 bg-slate-100 rounded-full border-2 border-dashed border-slate-300 flex items-center justify-center">
                      <div className="w-1 h-1 sm:w-2 sm:h-2 bg-slate-400 rounded-full" aria-label={`Stamp ${i} needed`}></div>
                    </div>
                  ))}
                </div>
